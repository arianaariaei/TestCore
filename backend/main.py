...
@app.post("/exams/")
async def create_exam(exam: ExamCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        db_exam = Exam(
            title=exam.title,
...
async def update_exam_results(exam_id: int, exam_update: ExamUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail="Failed to update exam results")


@app.get("/exams/")
async def get_user_exams(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
...
@app.get("/admin/exams/")
async def get_all_exams(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
   try:
       if not current_user.is_admin:
           raise HTTPException(status_code=403, detail="Admin access required")

       exams_with_users = (
           db.query(Exam, User.user_id, User.name, User.email, User.university)
...
async def get_user_exams_details(user_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        if not current_user.is_admin:
            raise HTTPException(status_code=403, detail="Admin access required")
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch user exam details")

@app.get("/reports/exams-by-subject")
async def get_exams_by_subject(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        subject_counts = (
            db.query(
                Exam.subject,
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch exam statistics by subject")


@app.get("/reports/users-exam-count")
async def get_users_exam_count(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        if not current_user.is_admin:
            raise HTTPException(status_code=403, detail="Admin access required")
        ...
        user_stats = (
            db.query(
                ...
                User.email,
                User.university,
                func.count(user_exams.c.exam_id).label('total_exams')
            )
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch user exam counts")